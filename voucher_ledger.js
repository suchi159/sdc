...
const proctorRequests = [
  { id: 1, proctor: 'VPC-102 (Sarah Jenkins)', amount: 50, reason: 'Upcoming large on-site cohort next week.' },
...
function renderTable() {
  ...
  filtered.forEach((v, index) => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="course-badge">${v.course}</span></td>
      <td><a href="#">${v.student}</a></td>
      <td><a href="#">${v.proctorId}</a></td>
      <td><span class="status-indicator ${statusClass}">${v.status}</span></td>
...
revokeSelectedBtn.addEventListener('click', () => {
  ...
  revokeModalText.textContent = `You are about to revoke ${selectedIds.size} vouchers. This action will wipe database strings and students will lose access.`;
