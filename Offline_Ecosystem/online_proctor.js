...
function navigateTo(view) {
  ...
  if (breadcrumbEl) breadcrumbEl.innerHTML = `<span class="breadcrumb-item">${titles[view] || view}</span>`;
...
async function handleLogout() {
  ...
  // Auto-fill the demo credentials so the proctor can sign straight back in.
  const emailEl = document.getElementById('login-email');
  const passEl = document.getElementById('login-password');
  if (emailEl) emailEl.value = 'sarah.jenkins@secureproctor.ai';
  if (passEl) passEl.value = 'training123';
}
...
function renderDashboard(data) {
  ...
  if (activeDiv) {
    if (data.liveSessions.length === 0) {
      activeDiv.innerHTML = `<div class="empty-state">No live classes currently running.</div>`;
    } else {
      activeDiv.innerHTML = data.liveSessions.map(s => `
        <div class="dash-list-item" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid var(--out-var);">
          ...
          <button class="btn-secondary" style="padding:6px 12px; min-height:0; font-size:12px;" onclick="navigateTo('monitoring')">Join</button>
        </div>
      `).join('');
    }
  }
  ...
  if (upcomingDiv) {
    if (data.upcomingSessions.length === 0) {
      upcomingDiv.innerHTML = `<div class="empty-state">No upcoming classes.</div>`;
    } else {
  ...
  if (voucherDiv) {
    ...
    voucherDiv.innerHTML = `
      ...
      <div style="margin-top:20px; text-align:center;">
        <button class="btn-secondary" onclick="requestOrPurchaseVouchers()">${canI('purchaseVouchers') ? 'Purchase Vouchers' : 'Request More Vouchers'}</button>
      </div>
    `;
  }
}
  ...
  tbody.innerHTML = filtered.map(c => {
    ...
    return `
      <tr>
        ...
        <td>${riskHtml}</td>
        <td>
          <button class="action-btn" title="Session Supervisor" onclick="openSessionSupervisor('${c.id}')"><i class="material-icons">visibility</i></button>
          <button class="action-btn" title="Take Action" onclick="openSessionSupervisor('${c.id}')"><i class="material-icons" style="color:var(--wrn);">warning</i></button>
        </td>
      </tr>
    `;
  }).join('');
  ...
  feed.innerHTML = alerts.map(a => `
    <div style="background:var(--sur); border:1px solid var(--out-var); border-radius:8px; padding:12px; margin-bottom:12px; border-left:4px solid var(--err);">
      ...
      <div style="display:flex; gap:8px;">
        <button class="btn-secondary" style="flex:1; min-height:28px; padding:4px; font-size:11px;" onclick="pushToast('Alert dismissed', 'info')">Dismiss</button>
        <button class="btn-primary" style="flex:1; min-height:28px; padding:4px; font-size:11px;" onclick="pushToast('Alert escalated', 'success')">Escalate</button>
      </div>
    </div>
  `).join('');
}
...
async function fetchSessions() {
  try {
    ...
    pushToast('Failed to load classes', 'error');
  }
}
...
function renderSessions() {
  ...
  if (filtered.length === 0) {
    grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:40px; color:var(--on-sur-var);">No classes found.</div>';
    return;
  }
    ...
    return `
      <div style="background:var(--sur); border:1px solid var(--out-var); border-radius:8px; padding:var(--spacing-lg); display:flex; flex-direction:column; gap:var(--spacing-md); cursor:pointer; transition:0.2s;" onclick="viewSessionDetails('${s.id}')" onmouseover="this.style.borderColor='var(--pri)'" onmouseout="this.style.borderColor='var(--out-var)'">
        ...
        <div style="margin-top:auto; display:flex; gap:var(--spacing-sm); justify-content:flex-end; border-top:1px solid var(--out-var); padding-top:var(--spacing-md);">
          ${s.status !== 'completed' ? `<button class="btn-secondary" onclick="event.stopPropagation(); showEditSessionModal('${s.id}')">Edit</button>` : ''}
          ${s.status === 'live' ? `<button class="btn-primary" onclick="event.stopPropagation(); navigateTo('monitoring')">Monitor</button>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
async function viewSessionDetails(id) {
  ...
  try {
    ...
    pushToast('Failed to load class details', 'error');
  }
}
      ...
      <tr style="border-bottom:1px solid var(--out-var);">
        ...
        <td style="padding:12px 16px;">
          <div style="display:flex; align-items:center; gap:6px;">
            ...
            ${!c.voucherCode ? `<button class="mdbtn btn-tonal" onclick="showRedeemByCodeModal('${c.id}')" style="padding:4px 8px; font-size:12px;">Redeem</button>
            <button class="mdbtn btn-text" title="Remind candidate to buy a voucher" onclick="remindCandidate('${c.id}')" style="padding:4px 6px;"><i class="material-icons" style="font-size:16px;">notifications_active</i></button>` : ''}
          </div>
        </td>
      </tr>
    `;
...
function allocateVouchers() {
  ...
  if(cbs.length === 0) { pushToast('Select candidates to assign vouchers', 'warning'); return; }
  ...
  pushToast(`Assigned vouchers to ${cbs.length} candidates`, 'success');
  renderSessionCandidates();
}
...
async function toggleSessionState() {
  ...
  if(s.status === 'live') {
    ...
    pushToast('Exam class ended.', 'success');
  } else if (s.status === 'completed') {
    return;
  } else {
    await apiFetch(`/sessions/${s.id}`, { method: 'PUT', body: { status: 'live' } });
    pushToast('Exam class started!', 'success');
    simulateLiveSession();
  }
...
function showCandidateDetail(id) {
  ...
  if (scenario === 1) {
    ...
    `;
    footerActions = `
      <button class="mdbtn btn-text" style="color:var(--err);" onclick="pushToast('Account Deactivated', 'error'); closeModal()">Deactivate Account</button>
    ...
    footerActions = `
      <button class="mdbtn btn-text" style="color:var(--err);" onclick="pushToast('Candidate Removed', 'error'); closeModal()">Remove Candidate</button>
      <button class="mdbtn btn-text" onclick="showReplaceVoucherModal('${c.id}')">Replace Voucher</button>
      <button class="btn-primary" onclick="redeemCandidateVoucher('${c.id}')">Redeem Voucher</button>
    `;
  }
  else {
    scenarioContent = `
    ...
    footerActions = `
      <button class="mdbtn btn-text" style="color:var(--err);" onclick="pushToast('Candidate Removed', 'error'); closeModal()">Remove Candidate</button>
      <button class="mdbtn btn-text" onclick="showRedeemByCodeModal('${c.id}')">Redeem Code</button>
      <button class="btn-primary" onclick="assignVoucherWithCheck('${c.id}', '${c.email}')">Assign Voucher</button>
    `;
  }
...
// Certificate course voucher pools — mirrors the seeded pools in the admin
// portal (admin_app.js) so the Create Class form behaves identically here.
const VOUCHER_POOLS = [
  { id: 'v-chef-001',  name: 'Professional Chef Certification', available: 6 },
  { id: 'v-fss-002',   name: 'Food Safety Standard',           available: 32 },
  { id: 'v-haccp-003', name: 'HACCP Level 3',                  available: 16 },
  { id: 'v-culin-004', name: 'Culinary Arts',                  available: 18 }
];

async function showCreateSessionModal() {
  let modules = [];
  try { modules = await apiFetch('/modules'); } catch (e) { modules = []; }
  const materialsHtml = (Array.isArray(modules) && modules.length)
    ? modules.map(m => `
        <label style="display:flex; align-items:center; gap:8px; font-size:13px; padding:8px 10px; border:1px solid var(--out-var); border-radius:8px; cursor:pointer;">
          <input type="checkbox" class="create-s-material-cb" value="${m.id}">
          <i class="material-icons" style="font-size:18px; color:var(--pri-dk);">${m.icon || 'menu_book'}</i>
          <span style="flex:1;">${m.title}</span>
          <span style="font-size:11px; color:var(--on-sur-var);">${m.duration || ''}</span>
        </label>`).join('')
    : '<div style="font-size:12px; color:var(--on-sur-var);">No learning materials available.</div>';

  const courseOptions = VOUCHER_POOLS.map(v => `<option value="${v.id}">${v.name}</option>`).join('');
  const poolOptions = VOUCHER_POOLS.map(v => `<option value="${v.id}">${v.name} (${v.available} Available)</option>`).join('');
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:flex; flex-direction:column; gap:var(--spacing-md);">
      ...
      <div class="inp-group">
        <label class="inp-label">Certificate Course</label>
        <select class="inp" id="create-s-program" onchange="updateCertCourseVouchers()">
          ${courseOptions}
        </select>
        <div id="create-s-voucher-status" style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-top:8px; font-size:13px;">
          <span id="create-s-voucher-count" style="color:var(--on-sur-var);"></span>
          <a href="#" onclick="requestMoreVouchers(); return false;" style="color:var(--pri); font-weight:600; text-decoration:none;">Request More</a>
        </div>
      </div>
      <div class="inp-group">
        <label class="inp-label">Voucher Pool</label>
        <select class="inp" id="create-s-voucher-pool" onchange="updateCertCourseVouchers('pool')">
          ${poolOptions}
        </select>
      </div>
      <div class="inp-group">
        <label class="inp-label">Proctoring Model</label>
        <select class="inp" id="create-s-flow" onchange="updateClassProctoringInfo()">
          <option value="A">Flow A - In-Classroom (Candidates pay for remote auto-toggle)</option>
          <option value="B">Flow B - Standard In-Classroom (Centralized)</option>
          <option value="C">Flow C - Hybrid / Online (B2B2C)</option>
        </select>
      </div>
      <div id="create-s-proctoring-info" style="background:var(--sur-var); padding:12px; border-radius:8px; font-size:12px; color:var(--on-sur-var);">
        Flow A: Candidates will take the exam in a designated classroom. If they fail, they can purchase a remote proctoring upgrade individually (Decentralized Financials).
      </div>
      ...
      <div class="inp-group">
        <label class="inp-label">Learning Materials</label>
        <div style="display:flex; flex-direction:column; gap:8px;">${materialsHtml}</div>
      </div>
  ...
  updateCertCourseVouchers();
}

// Keeps the Certificate Course and Voucher Pool selects in sync and surfaces
// the available voucher count for the chosen certification. `source` indicates
// which select fired the change ('pool' = Voucher Pool dropdown).
function updateCertCourseVouchers(source) {
  const courseSel = document.getElementById('create-s-program');
  const poolSel = document.getElementById('create-s-voucher-pool');
  const countEl = document.getElementById('create-s-voucher-count');
  if (!courseSel || !poolSel) return;

  if (source === 'pool') {
    courseSel.value = poolSel.value;
  } else {
    poolSel.value = courseSel.value;
  }

  const voucher = VOUCHER_POOLS.find(v => v.id === courseSel.value);
  if (countEl) {
    if (voucher) {
      const available = voucher.available || 0;
      const color = available > 0 ? 'var(--suc)' : 'var(--err)';
...
// "Request More" link from the Certificate Course field — defers to the shared
// purchase-or-request flow (gated by the user's purchaseVouchers permission).
function requestMoreVouchers() {
  requestOrPurchaseVouchers();
}

// Updates the Proctoring Model explainer to match the selected flow.
function updateClassProctoringInfo() {
  const flow = document.getElementById('create-s-flow');
  const info = document.getElementById('create-s-proctoring-info');
  if (!flow || !info) return;
  if (flow.value === 'A') {
    info.innerHTML = "Flow A: Candidates will take the exam in a designated classroom. If they fail, they can purchase a remote proctoring upgrade individually (Decentralized Financials).";
  } else if (flow.value === 'B') {
    info.innerHTML = "Flow B: Standard in-classroom proctoring. All vouchers are bulk-purchased by the institution. No remote options (Centralized Procurement).";
  } else if (flow.value === 'C') {
    info.innerHTML = "Flow C: Hybrid online model. Candidates take the exam remotely with an AI/Human proctor. Costs charged to corporate card (B2B2C).";
  }
}
...
async function saveNewClass() {
  ...
  const learningMaterials = Array.from(document.querySelectorAll('.create-s-material-cb:checked')).map(cb => parseInt(cb.value));

  // Certificate Course / Voucher Pool selects are keyed by voucher pool id.
  const poolId = (document.getElementById('create-s-voucher-pool') || {}).value || '';
  const pool = VOUCHER_POOLS.find(v => v.id === poolId);
  const course = pool ? pool.name : '';
  const vouchersAvailable = pool ? pool.available : 0;
  const proctoringFlow = (document.getElementById('create-s-flow') || {}).value || 'A';
  ...
  try {
    const res = await apiFetch('/sessions', {
      method: 'POST',
      body: { name, examDate, startTime, duration, vouchersAvailable, learningMaterials, course, voucherPoolId: poolId, proctoringFlow }
    });
    pushToast('Class created successfully', 'success');
    closeModal();
    fetchSessions(); // Refresh grid
    setTimeout(() => { showAddCandidateModal(res.id || 'sess_' + Date.now()); }, 300); // Trigger add candidate flow
  } catch (e) {
    pushToast('Failed to create class', 'error');
  }
}
...
function showEditSessionModal(id) {
  ...
  document.getElementById('modal-footer').innerHTML = `
    <button class="btn-secondary" onclick="closeModal()">Cancel</button>
    <button class="btn-primary" onclick="saveSessionEdit('${id}')">Save Changes</button>
  `;
  document.getElementById('modal-overlay').classList.remove('hidden');
}
...
async function saveSessionEdit(id) {
  ...
  try {
    ...
    pushToast('Class updated successfully', 'success');
    closeModal();
    fetchSessions(); // Refresh grid
  } catch(e) {
    pushToast('Failed to update class', 'error');
  }
}
...
function renderCandidatesTable() {
  ...
  tbody.innerHTML = filtered.map(c => `
    <tr>
      ...
      <td><span class="badge" style="font-size:11px; padding:2px 8px; border-radius:4px;">${c.examStatus.replace('_', ' ')}</span></td>
      <td>
        ${c.voucherCode ? 
...
function renderIncidentTab(tab) {
  ...
  if (tab === 'aiflags') {
    container.innerHTML = `
      <div class="proctor-table-wrapper">
        <table class="proctor-table">
          <thead><tr><th>Time</th><th>Candidate</th><th>Class</th><th>Alert Type</th><th>Confidence</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            ${inc.aiFlags.map(f => `
              <tr>
                ...
                <td>${f.confidence}%</td>
                <td><span class="badge" style="font-size:11px;">${f.status}</span></td>
                <td>
                  <div style="display:flex; gap:var(--spacing-sm);">
                    <button class="btn-secondary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="resolveAiFlag('${f.id}', 'dismissed')">Dismiss</button>
                    <button class="btn-primary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="resolveAiFlag('${f.id}', 'escalated')">Escalate</button>
                    <button class="btn-secondary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="reviewIncident('aiflags', '${f.id}')">Review</button>
                  </div>
                </td>
              </tr>
            `).join('')}
    ...
    container.innerHTML = `
      <div class="proctor-table-wrapper">
        <table class="proctor-table">
          <thead><tr><th>Flagged At</th><th>Candidate</th><th>Class</th><th>Reason</th><th>Flagged By</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            ${inc.flaggedCases.map(fc => `
              <tr>
                ...
                <td>${fc.reason}</td>
                <td>${fc.flaggedBy}</td>
                <td><span class="badge" style="font-size:11px;">${fc.status}</span></td>
                <td>
                  <button class="btn-secondary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="reviewIncident('flagged', '${fc.id}')">Review</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
    ...
    container.innerHTML = `
      <div class="proctor-table-wrapper">
        <table class="proctor-table">
          <thead><tr><th>Requested</th><th>Candidate</th><th>Class</th><th>Reason</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            ${inc.retakeRequests.map(r => `
              <tr>
                ...
                <td>${r.sessionName}</td>
                <td>${r.reason}</td>
                <td><span class="badge" style="font-size:11px;">${r.status}</span></td>
                <td>
                  <div style="display:flex; gap:var(--spacing-sm); align-items:center;">
                    ${r.status === 'pending' ? `
                      <button class="action-btn" style="color:var(--suc);" onclick="handleRetake('${r.id}', 'approved')"><i class="material-icons">check</i></button>
                      <button class="action-btn" style="color:var(--err);" onclick="handleRetake('${r.id}', 'denied')"><i class="material-icons">close</i></button>
                    ` : ''}
                    <button class="btn-secondary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="reviewIncident('retakes', '${r.id}')">Review</button>
                  </div>
                </td>
              </tr>
            `).join('')}
    ...
    container.innerHTML = `
      <div class="proctor-table-wrapper">
        <table class="proctor-table">
          <thead><tr><th>Escalated At</th><th>Candidate</th><th>Class</th><th>Evidence Flags</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            ${inc.malpracticeCases.map(mp => `
              <tr>
                ...
                <td><span class="badge" style="font-size:11px; font-family:var(--font-mono);">${mp.evidence.join(', ') || 'None'}</span></td>
                <td><span class="badge" style="font-size:11px;">${mp.status.replace('_', ' ')}</span></td>
                <td>
                  <button class="btn-secondary" style="padding:4px 8px; min-height:0; font-size:12px;" onclick="reviewIncident('malpractice', '${mp.id}')">Review</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
...
function reviewIncident(type, id) {
  ...
  modalHeader.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
      ...
      <button class="action-btn" onclick="closeModal()" aria-label="Close modal">
    ...
    <div style="display:grid; grid-template-columns:1.2fr 1fr; gap:20px; text-align:left;">
      ...
      <div style="display:flex; flex-direction:column; gap:20px;">
        ...
        <div>
          ...
          <div style="background:var(--sur); border:1px solid var(--out-var); border-radius:8px; padding:12px; display:flex; flex-direction:column; gap:8px; font-size:13px;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:4px; border-top:1px solid rgba(0,0,0,0.04); padding-top:8px;">
              ...
              <div><strong>Learning Material:</strong> ${candidate.course || 'N/A'}</div>
            </div>

            <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
              <div><strong>Exam Status:</strong> 
                <span class="badge" style="font-size:10px; padding:1px 6px; border-radius:4px;">
        ...
        <div>
          ...
          <div style="background:var(--sur); border:1px solid var(--out-var); border-radius:8px; padding:12px; display:flex; flex-direction:column; gap:8px; font-size:13px;">
            <div style="font-weight:700; color:var(--pri-dk);">${session.name || incident.sessionName || 'Unknown Class'}</div>
            ...
            <div><strong>Class Size:</strong> ${session.candidateCount || 0} candidates</div>
          </div>
        </div>
  ...
  modalFooter.innerHTML = `
    <div style="display:flex; justify-content:flex-end; gap:var(--spacing-sm); width:100%;">
      <button class="btn-secondary" onclick="closeModal()">Cancel</button>
      <button class="btn-primary" onclick="saveIncidentReview('${type}', '${id}')">Save Changes</button>
    </div>
  `;
...
async function fetchReports() {
  try {
    ...
    document.getElementById('reports-kpi-grid').innerHTML = `
      <div class="kpi-card"><div class="kpi-val">${data.overview.totalSessions}</div><div class="kpi-lbl">Total Classes</div></div>
    ...
    document.getElementById('reports-session-breakdown').innerHTML = data.sessionBreakdown.map(s => `
      <tr>
        ...
        <td><span class="badge" style="font-size:11px;">${s.status}</span></td>
  ...
  try {
    ...
    document.getElementById('earnings-sessions-body').innerHTML = data.sessions.map(s => `
      <tr>
        ...
        <td><span class="badge" style="font-size:11px;">${s.payoutStatus}</span></td>
      </tr>
    `).join('');
...
// Proctor code display + renew/deactivate (in-class certification)
const PCODE_STATUS_MAP = {
  active: { lbl: 'Active', bg: 'rgba(20,108,46,0.1)', col: 'var(--suc)' },
  expiring: { lbl: 'Expiring Soon', bg: 'rgba(125,87,0,0.1)', col: 'var(--wrn)' },
  expired: { lbl: 'Expired', bg: 'rgba(179,38,30,0.1)', col: 'var(--err)' },
  deactivated: { lbl: 'Deactivated', bg: 'rgba(71,85,105,0.1)', col: 'var(--on-sur-var)' }
};

async function renderProctorCodeCard() {
  let code = STATE.proctorCode;
  try { const r = await apiFetch('/proctor-code'); code = r.code; STATE.proctorCode = code; } catch (e) {}
  const section = document.getElementById('settings-pcode-section');
  if (!code) { if (section) section.style.display = 'none'; return; }
  if (section) section.style.display = '';

  const numEl = document.getElementById('settings-pcode-number');
  if (numEl) numEl.textContent = code.codeId || '—';
  const courseEl = document.getElementById('settings-pcode-course');
  if (courseEl) courseEl.textContent = code.course || '—';
  const issued = document.getElementById('settings-pcode-issued');
  if (issued) issued.textContent = code.issuedAt ? new Date(code.issuedAt).toLocaleDateString() : '—';
  const expiry = document.getElementById('settings-pcode-expiry');
  if (expiry) expiry.textContent = code.expiresAt ? new Date(code.expiresAt).toLocaleDateString() : '—';

  const statusEl = document.getElementById('settings-pcode-status');
  const badge = document.getElementById('settings-pcode-badge');
  const daysEl = document.getElementById('settings-pcode-daysleft');
  const st = PCODE_STATUS_MAP[code.status] || PCODE_STATUS_MAP.active;
  if (statusEl) statusEl.textContent = st.lbl;
  if (badge) { badge.style.background = st.bg; badge.style.color = st.col; badge.style.borderColor = st.col; }
  if (daysEl) {
    if (code.status === 'deactivated') daysEl.textContent = '(deactivated)';
    else if (typeof code.daysToExpiry === 'number') daysEl.textContent = code.daysToExpiry >= 0 ? `(${code.daysToExpiry} days left)` : `(expired ${Math.abs(code.daysToExpiry)} days ago)`;
    else daysEl.textContent = '';
  }
}

async function renewProctorCode() {
  try {
    await apiFetch('/proctor-code/renew', { method: 'POST', body: {} });
    await renderProctorCodeCard();
    pushToast('SDC Proctor ID renewed for another year', 'success');
  } catch (e) {
    pushToast('Failed to renew SDC Proctor ID', 'error');
  }
}

async function deactivateProctorCode() {
  if (!confirm('Deactivate your SDC Proctor ID? You will need to renew it before proctoring again.')) return;
  try {
    await apiFetch('/proctor-code/deactivate', { method: 'POST', body: {} });
    await renderProctorCodeCard();
    pushToast('SDC Proctor ID deactivated', 'success');
  } catch (e) {
    pushToast('Failed to deactivate SDC Proctor ID', 'error');
  }
}

// License + permissions display (F4 / F7)
async function renderLicenseCard() {
  let lic = STATE.license;
  try { const r = await apiFetch('/license'); lic = r.license; STATE.license = lic; } catch (e) {}
  if (!lic) return;
  const numEl = document.getElementById('settings-license-number');
  if (numEl) numEl.textContent = lic.number || '—';
  const issued = document.getElementById('settings-license-issued');
  if (issued) issued.textContent = lic.issuedAt ? new Date(lic.issuedAt).toLocaleDateString() : '—';
  const expiry = document.getElementById('settings-license-expiry');
  if (expiry) expiry.textContent = lic.expiresAt ? new Date(lic.expiresAt).toLocaleDateString() : '—';
  const statusEl = document.getElementById('settings-license-status');
  const badge = document.getElementById('settings-license-badge');
  const daysEl = document.getElementById('settings-license-daysleft');
  const statusMap = {
    active: { lbl: 'Active', bg: 'rgba(20,108,46,0.1)', col: 'var(--suc)' },
    expiring: { lbl: 'Expiring Soon', bg: 'rgba(125,87,0,0.1)', col: 'var(--wrn)' },
    expired: { lbl: 'Expired', bg: 'rgba(179,38,30,0.1)', col: 'var(--err)' }
  };
  const st = statusMap[lic.status] || statusMap.active;
  if (statusEl) statusEl.textContent = st.lbl;
  if (badge) { badge.style.background = st.bg; badge.style.color = st.col; badge.style.borderColor = st.col; }
  if (daysEl && typeof lic.daysToExpiry === 'number') {
    daysEl.textContent = lic.daysToExpiry >= 0 ? `(${lic.daysToExpiry} days left)` : `(expired ${Math.abs(lic.daysToExpiry)} days ago)`;
  }
}

async function renderPermissionsCard() {
  let perms = STATE.permissions, grantedBy = STATE.permissionsGrantedBy;
  try { const r = await apiFetch('/permissions'); perms = r.permissions || {}; grantedBy = r.grantedBy; STATE.permissions = perms; STATE.permissionsGrantedBy = grantedBy; } catch (e) {}
  const gb = document.getElementById('settings-rights-grantedby');
  if (gb) gb.textContent = grantedBy || 'your organization admin';
  const list = document.getElementById('settings-permissions-list');
  if (!list) return;
  const labels = {
    createClasses: 'Create & manage classes',
    manageEnrollments: 'Manage candidate enrollments',
    assignVouchers: 'Assign / redeem vouchers',
    purchaseVouchers: 'Purchase vouchers (same as admin)'
  };
  list.innerHTML = Object.keys(labels).map(k => {
    const on = !!(perms && perms[k]);
    return `<div class="preference-item" style="margin-top:8px;">
      <div class="pref-desc"><h4 style="font-size:14px;">${labels[k]}</h4></div>
      <div class="bdg" style="background:${on ? 'rgba(20,108,46,0.1)' : 'rgba(142,138,148,0.12)'}; color:${on ? 'var(--suc)' : 'var(--on-sur-var)'}; border:1px solid ${on ? 'rgba(20,108,46,0.25)' : 'var(--out-var)'};">
        <i class="material-icons" style="font-size:14px;">${on ? 'check_circle' : 'lock'}</i> ${on ? 'Granted' : 'Not granted'}
      </div>
    </div>`;
  }).join('');
}

// F4 — go to license renewal (reuses the existing proctor_auth flow)
function goToLicenseRenewal() {
  window.location.href = '/proctor_auth.html?mode=renew';
}
    ...
    <div style="display:flex; flex-direction:column; gap:24px; padding-bottom:8px;">
      ...
      <div>
        ...
        <div style="display:flex; flex-direction:column; gap:8px;">
          <button class="btn-secondary" style="justify-content:flex-start; padding:12px 16px; font-weight:500;" onclick="pushToast('Warning dispatched', 'success'); closeModal();">
        ...
        <div style="display:flex; gap:12px; margin-top:12px;">
          <input class="inp" type="text" placeholder="Type a custom warning message to candidate..." style="flex:1;">
          <button class="btn-primary" onclick="pushToast('Custom warning sent', 'success'); closeModal();">Send</button>
        </div>
      </div>
      ...
      <div>
        ...
        <div style="display:flex; gap:12px;">
          <button class="btn-secondary" style="flex:1; justify-content:center; padding:12px;" onclick="pushToast('Exam paused', 'info'); closeModal();">
            <i class="material-icons" style="margin-right:8px;">pause</i> Pause Exam
          </button>
          <button class="btn-secondary" style="flex:1; justify-content:center; padding:12px; color:var(--err); border-color:var(--err);" onclick="pushToast('Feed suspended', 'error'); closeModal();">
      ...
      <div>
        ...
        <div style="display:flex; gap:12px;">
          <button class="btn-secondary" style="flex:1; justify-content:center; padding:12px;" onclick="pushToast('Escalated to IT Support', 'info'); closeModal();">
            <i class="material-icons" style="margin-right:8px;">build</i> IT Support
          </button>
          <button class="btn-secondary" style="flex:1; justify-content:center; padding:12px;" onclick="pushToast('Alerted Chairman', 'info'); closeModal();">
...
async function showAddCandidateModal(sessionId) {
  ...
  const liveSession = STATE.data.currentSession && STATE.data.currentSession.id === sessionId && STATE.data.currentSession.status === 'live';

  document.getElementById('modal-header').innerHTML = `<h2 style="font-size:18px;">Add Candidates to Class</h2>`;
  document.getElementById('modal-body').innerHTML = `
    <div style="display:flex; flex-direction:column; gap:16px;">
      ${liveSession ? `<div style="background:rgba(20,108,46,0.1); border:1px solid rgba(20,108,46,0.25); color:var(--suc); border-radius:8px; padding:10px 12px; font-size:12px;"><i class="material-icons" style="font-size:14px; vertical-align:middle;">bolt</i> This class is live — candidates can still be added at any time.</div>` : ''}
      <div style="display:flex; gap:8px;">
        ...
        ${availableCands.map(c => {
          ...
          <div class="add-cand-item" data-name="${c.name.toLowerCase()}" data-email="${c.email.toLowerCase()}" data-course="${c.course || ''}" data-batch="${batch}" style="display:flex; align-items:center; justify-content:space-between; padding:12px; background:var(--sur); border:1px solid var(--out-var); border-radius:8px;">
            <div style="display:flex; align-items:center; gap:12px;">
              ...
              <div>
                <div style="font-weight:600; font-size:14px;">${c.name}</div>
                <div style="font-size:12px; color:var(--on-sur-var);">${c.course || 'No Learning Material'} · Batch ${batch || 'N/A'}</div>
              </div>
            </div>
            <button class="btn-secondary" style="padding:4px 12px; font-size:13px;" onclick="addCandidateToSession('${c.id}', '${sessionId}', this)">Add</button>
          </div>
          `;
        }).join('')}
      </div>
  ...
  document.getElementById('modal-footer').innerHTML = `
    <div style="display:flex; justify-content:space-between; width:100%;">
      <button class="btn-secondary" onclick="showCreateCandidateModal('${sessionId}')"><i class="material-icons">person_add</i> Add New Candidate</button>
      <button class="btn-primary" onclick="closeModal(); viewSessionDetails('${sessionId}');">Done</button>
    </div>
  `;
...
function addCandidateToSession(candId, sessionId, btnEl) {
  ...
  // F5 — adding to a live (ongoing) class is allowed; surface it explicitly.
  const s = STATE.data.currentSession;
  if (s && s.status === 'live') {
    pushToast('Candidate added to the ongoing (live) class', 'success');
  } else {
...
// ============================================================================
// PROCTOR CONTEXT — permissions, license, notifications (loaded after gate)
// ============================================================================
STATE.permissions = {};
STATE.license = null;
STATE.notifications = [];

async function loadProctorContext() {
  try {
    const [perm, lic, ntfs] = await Promise.all([
      apiFetch('/permissions').catch(() => ({})),
      apiFetch('/license').catch(() => ({})),
      apiFetch('/notifications').catch(() => [])
    ]);
    STATE.permissions = perm.permissions || {};
    STATE.permissionsGrantedBy = perm.grantedBy || null;
    STATE.license = lic.license || null;
    STATE.notifications = Array.isArray(ntfs) ? ntfs : [];
    renderNotificationBell();
  } catch (e) { /* non-fatal */ }
}

// Permission gate helper (F7)
function canI(permKey) {
  return !!(STATE.permissions && STATE.permissions[permKey]);
}
...
function renderNotificationBell() {
  let bell = document.getElementById('sp-notif-bell-wrap');
  if (!bell) {
    bell = document.createElement('div');
    bell.id = 'sp-notif-bell-wrap';
    bell.style.cssText = 'position:fixed; top:16px; right:20px; z-index:900;';
    bell.innerHTML = `
      <button id="sp-notif-bell" class="action-btn" onclick="toggleNotifPanel()" title="Notifications"
        style="background:var(--sur); border:1px solid var(--out-var); border-radius:50%; width:42px; height:42px; box-shadow:var(--shadow-sm); position:relative;">
        <i class="material-icons">notifications</i>
        <span id="sp-notif-count" style="display:none; position:absolute; top:-4px; right:-4px; background:var(--err); color:#fff; font-size:10px; font-weight:800; min-width:16px; height:16px; line-height:16px; border-radius:8px; padding:0 4px;"></span>
      </button>
      <div id="sp-notif-panel" class="hidden" style="position:absolute; top:50px; right:0; width:340px; max-height:440px; overflow-y:auto; background:var(--sur); border:1px solid var(--out-var); border-radius:12px; box-shadow:var(--shadow-lg);"></div>`;
    document.body.appendChild(bell);
  }
  const unread = STATE.notifications.filter(n => !n.read).length;
  const countEl = document.getElementById('sp-notif-count');
  if (countEl) {
...
function renderNotifPanel() {
  ...
  const items = STATE.notifications.length ? STATE.notifications.map(n => `
    <div onclick="markNotifRead('${n.id}')" style="display:flex; gap:12px; padding:12px 16px; border-bottom:1px solid var(--out-var); cursor:pointer; background:${n.read ? 'transparent' : 'rgba(249,173,0,0.06)'};">
      <i class="material-icons" style="color:var(--pri-dk); font-size:20px;">${n.icon || 'notifications'}</i>
      <div style="flex:1;">
        <div style="font-weight:600; font-size:13px;">${n.title}</div>
        <div style="font-size:12px; color:var(--on-sur-var); margin:2px 0;">${n.message}</div>
        <div style="font-size:11px; color:var(--out);">${timeAgo(n.createdAt)}</div>
      </div>
    </div>`).join('') : `<div style="padding:24px; text-align:center; color:var(--on-sur-var);">No notifications.</div>`;
  panel.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-bottom:1px solid var(--out-var);">
      <strong style="font-size:14px;">Notifications</strong>
      <button class="mdbtn btn-text" style="font-size:12px; padding:2px 6px;" onclick="markAllNotifsRead(event)">Mark all read</button>
    </div>
    ${items}`;
}
...
async function markNotifRead(id) {
  try {
    STATE.notifications = await apiFetch(`/notifications/${id}/read`, { method: 'PUT', body: { read: true } });
  } catch (e) {}
  renderNotificationBell();
}
...
async function markAllNotifsRead(e) {
  ...
  try {
    STATE.notifications = await apiFetch('/notifications/all/read', { method: 'PUT', body: {} });
  } catch (e) {}
  renderNotificationBell();
}
...
// ============================================================================
// F1 — Redeem a candidate's voucher (blocked if learning material started)
// ============================================================================
async function redeemCandidateVoucher(candidateId) {
  try {
    const res = await apiFetch(`/candidates/${candidateId}/redeem-voucher`, { method: 'POST', body: {} });
    if (res.error === 'learning_started') {
      pushToast(res.message, 'warning');
      return;
    }
    if (res.error) { pushToast(res.message || res.error, 'error'); return; }
    pushToast('Voucher redeemed successfully', 'success');
    closeModal();
    refreshNotifications();
    if (STATE.currentSessionId) viewSessionDetails(STATE.currentSessionId);
  } catch (e) {
    pushToast('Failed to redeem voucher', 'error');
  }
}

// ============================================================================
// F2 — Replace a candidate's voucher by request (auto-verified by rules)
// ============================================================================
function showReplaceVoucherModal(candidateId) {
  document.getElementById('modal-header').innerHTML = `<h2 style="font-size:18px;">Replace Voucher (by request)</h2>`;
  document.getElementById('modal-body').innerHTML = `
    <p style="font-size:13px; color:var(--on-sur-var); margin-bottom:16px;">
      Enter the replacement voucher code the candidate has requested. The system will auto-verify it
      (exists, unused, not tied to another candidate, matching certification) before swapping.
    </p>
    <div class="inp-group">
      <label class="inp-label">New Voucher Code</label>
      <input class="inp" type="text" id="replace-voucher-code" placeholder="e.g. VCH-A1009">
    </div>
    <div id="replace-verify-result" style="margin-top:12px;"></div>
  `;
  document.getElementById('modal-footer').innerHTML = `
    <button class="btn-secondary" onclick="closeModal()">Cancel</button>
    <button class="btn-primary" onclick="submitReplaceVoucher('${candidateId}')">Verify & Replace</button>
  `;
  document.getElementById('modal-overlay').classList.remove('hidden');
}

async function submitReplaceVoucher(candidateId) {
  const newVoucherId = document.getElementById('replace-voucher-code').value.trim();
  if (!newVoucherId) { pushToast('Enter a voucher code', 'warning'); return; }
  const resultDiv = document.getElementById('replace-verify-result');
  try {
    const res = await apiFetch('/vouchers/replace', { method: 'POST', body: { candidateId, newVoucherId } });
    if (res.checks) {
      resultDiv.innerHTML = res.checks.map(ck => `
        <div style="display:flex; align-items:center; gap:8px; font-size:13px; margin-bottom:4px;">
          <i class="material-icons" style="font-size:16px; color:${ck.pass ? 'var(--suc)' : 'var(--err)'};">${ck.pass ? 'check_circle' : 'cancel'}</i>
          ${ck.rule}
        </div>`).join('');
    }
    if (res.success && res.verified) {
      pushToast('Voucher verified and replaced', 'success');
      refreshNotifications();
      setTimeout(() => { closeModal(); if (STATE.currentSessionId) viewSessionDetails(STATE.currentSessionId); }, 800);
    } else {
      pushToast(res.message || 'Verification failed', 'error');
    }
  } catch (e) {
    pushToast('Failed to replace voucher', 'error');
  }
}

// ============================================================================
// F3 — Duplicate email voucher check (notify if email already has a voucher)
// ============================================================================
async function checkEmailHasVoucher(email) {
  try {
    const res = await apiFetch(`/vouchers/check-email?email=${encodeURIComponent(email)}`);
    return res;
  } catch (e) { return { hasVoucher: false, vouchers: [] }; }
}

async function assignVoucherWithCheck(candidateId, email) {
  if (email) {
    const check = await checkEmailHasVoucher(email);
    if (check.hasVoucher) {
      const v = check.vouchers[0];
      pushToast(`Heads up: ${email} already has voucher ${v.voucherCode} (${v.voucherStatus}).`, 'warning');
    }
  }
  const code = 'SDC-' + Math.random().toString(36).substr(2, 6).toUpperCase();
  await apiFetch(`/candidates/${candidateId}`, { method: 'PUT', body: { voucherCode: code } }).catch(console.error);
  pushToast('Voucher assigned', 'success');
  closeModal();
  if (STATE.currentSessionId) viewSessionDetails(STATE.currentSessionId);
}

// ============================================================================
// Redeem a candidate-shared code (simulated validation — mirrors the in-class
// proctor). Accepts any well-formed VCH-style code; seeded codes demo errors.
// ============================================================================
function validateVoucherCode(code) {
  const c = (code || '').trim().toUpperCase();
  if (!/^[A-Z]{2,4}-[A-Z0-9-]{3,}$/.test(c)) return { ok: false, message: 'Please enter a valid voucher code.' };
  if (c === 'VCH-USED')  return { ok: false, message: 'This voucher has already been redeemed.' };
  if (c === 'VCH-OTHER') return { ok: false, message: 'This voucher is tied to another candidate.' };
  if (c === 'VCH-WRONG') return { ok: false, message: "Voucher certification doesn't match this class." };
  return { ok: true, code: c };
}

function showRedeemByCodeModal(candidateId) {
  document.getElementById('modal-header').innerHTML = `<h2 style="font-size:18px;">Redeem Voucher</h2>`;
  document.getElementById('modal-body').innerHTML = `
    <p style="font-size:13px; color:var(--on-sur-var); margin-bottom:16px;">
      Enter the voucher code the candidate bought and shared with you. We'll verify it before redeeming.
    </p>
    <div class="inp-group">
      <label class="inp-label">Voucher code from candidate</label>
      <input class="inp" type="text" id="redeem-by-code-input" placeholder="e.g. VCH-B0007">
    </div>
    <div id="redeem-by-code-result" style="margin-top:12px;"></div>
  `;
  document.getElementById('modal-footer').innerHTML = `
    <button class="btn-secondary" onclick="closeModal()">Cancel</button>
    <button class="btn-primary" onclick="submitRedeemByCode('${candidateId}')"><i class="material-icons" style="font-size:18px;">verified</i> Verify & Redeem</button>
  `;
  document.getElementById('modal-overlay').classList.remove('hidden');
}

async function submitRedeemByCode(candidateId) {
  const input = document.getElementById('redeem-by-code-input');
  const out = document.getElementById('redeem-by-code-result');
  const res = validateVoucherCode(input ? input.value : '');
  if (!res.ok) {
    if (out) out.innerHTML = `<div style="display:flex; align-items:flex-start; gap:8px; font-size:13px; color:var(--err);"><i class="material-icons" style="font-size:16px;">error</i><span>${res.message}</span></div>`;
    return;
  }
  await apiFetch(`/candidates/${candidateId}`, { method: 'PUT', body: { voucherCode: res.code, voucherStatus: 'redeemed' } }).catch(console.error);
  const c = STATE.data && STATE.data.currentSession && STATE.data.currentSession.candidates.find(x => x.id === candidateId);
  if (c) { c.voucherCode = res.code; c.voucherStatus = 'redeemed'; }
  pushToast(`Voucher ${res.code} redeemed${c ? ' for ' + c.name : ''}.`, 'success');
  closeModal();
  if (STATE.currentSessionId) viewSessionDetails(STATE.currentSessionId);
}

// ============================================================================
// Reminders (simulated send) — nudge candidates who haven't bought a voucher.
// ============================================================================
function remindCandidate(candidateId) {
  const s = STATE.data && STATE.data.currentSession;
  const c = s && s.candidates.find(x => x.id === candidateId);
  pushToast(`Reminder sent to ${c ? c.name : 'candidate'} to purchase a voucher.`, 'success');
}

function remindAllUnassigned() {
  const s = STATE.data && STATE.data.currentSession;
  if (!s) { pushToast('Open a class to remind its candidates.', 'warning'); return; }
  const n = (s.candidates || []).filter(c => {
    const v = (c.voucherStatus || '').toLowerCase();
    return !c.voucherCode && v !== 'redeemed' && v !== 'activated' && v !== 'assigned' && v !== 'pending';
  }).length;
  if (!n) { pushToast('All candidates already have vouchers.', 'info'); return; }
  pushToast(`Reminders sent to ${n} candidate${n > 1 ? 's' : ''}.`, 'success');
}

// ============================================================================
// F6 — Add a brand-new candidate to the candidate list (and optionally a class)
// ============================================================================
function showCreateCandidateModal(sessionId) {
  document.getElementById('modal-header').innerHTML = `<h2 style="font-size:18px;">Add New Candidate</h2>`;
  document.getElementById('modal-body').innerHTML = `
    <div style="display:flex; flex-direction:column; gap:var(--spacing-md);">
      <div class="inp-group"><label class="inp-label">Full Name</label><input class="inp" type="text" id="new-cand-name"></div>
      <div class="inp-group"><label class="inp-label">Email</label><input class="inp" type="email" id="new-cand-email"></div>
      <div style="display:flex; gap:16px;">
        <div class="inp-group" style="flex:1;"><label class="inp-label">Roll No (optional)</label><input class="inp" type="text" id="new-cand-roll"></div>
        <div class="inp-group" style="flex:1;"><label class="inp-label">Course / Subject</label><input class="inp" type="text" id="new-cand-subject"></div>
      </div>
      <div id="new-cand-emailwarn" style="font-size:12px;"></div>
    </div>`;
  document.getElementById('modal-footer').innerHTML = `
    <button class="btn-secondary" onclick="closeModal()">Cancel</button>
    <button class="btn-primary" onclick="submitNewCandidate('${sessionId || ''}')">Save to Candidate List</button>`;
  document.getElementById('modal-overlay').classList.remove('hidden');

  const emailEl = document.getElementById('new-cand-email');
  emailEl.addEventListener('blur', async () => {
    const warn = document.getElementById('new-cand-emailwarn');
    if (!emailEl.value) { warn.innerHTML = ''; return; }
    const check = await checkEmailHasVoucher(emailEl.value);
    if (check.hasVoucher) {
      const v = check.vouchers[0];
      warn.innerHTML = `<span style="color:var(--wrn);"><i class="material-icons" style="font-size:14px; vertical-align:middle;">warning</i> This email already has voucher ${v.voucherCode} (${v.voucherStatus}).</span>`;
    } else {
      warn.innerHTML = `<span style="color:var(--suc);"><i class="material-icons" style="font-size:14px; vertical-align:middle;">check_circle</i> No existing voucher for this email.</span>`;
    }
  });
}

async function submitNewCandidate(sessionId) {
  const name = document.getElementById('new-cand-name').value.trim();
  const email = document.getElementById('new-cand-email').value.trim();
  const rollNo = document.getElementById('new-cand-roll').value.trim();
  const subject = document.getElementById('new-cand-subject').value.trim();
  if (!name || !email) { pushToast('Name and email are required', 'error'); return; }
  try {
    const res = await apiFetch('/candidates', { method: 'POST', body: { name, email, rollNo, subject, sessionId: sessionId || null } });
    if (res.error) { pushToast(res.error, 'error'); return; }
    if (res.duplicateEmail) {
      pushToast(`Saved. Note: ${email} matches existing candidate ${res.duplicateEmail.name}.`, 'warning');
    } else {
      pushToast('Candidate saved to candidate list', 'success');
    }
    refreshNotifications();
    closeModal();
    if (sessionId) { viewSessionDetails(sessionId); }
...
// ============================================================================
// F7 — Purchase vouchers (only with delegated 'purchaseVouchers' right)
// ============================================================================
function showPurchaseVoucherModal() {
  if (!canI('purchaseVouchers')) {
    pushToast('You do not have permission to purchase vouchers. Ask your org admin to grant it.', 'warning');
    return;
  }
  document.getElementById('modal-header').innerHTML = `<h2 style="font-size:18px;">Purchase Vouchers</h2>`;
  document.getElementById('modal-body').innerHTML = `
    <p style="font-size:12px; color:var(--on-sur-var); margin-bottom:16px;">
      <i class="material-icons" style="font-size:14px; vertical-align:middle; color:var(--suc);">verified_user</i>
      Delegated admin right active — you can purchase like an organization admin.
    </p>
    <div style="display:flex; flex-direction:column; gap:var(--spacing-md);">
      <div class="inp-group">
        <label class="inp-label">Voucher Type</label>
        <select class="inp" id="purchase-type">
          <option value="In-House">In-House (In-Class)</option>
          <option value="Internet Proctored">Internet Proctored (Online)</option>
        </select>
      </div>
      <div class="inp-group">
        <label class="inp-label">Quantity</label>
        <input class="inp" type="number" id="purchase-qty" value="10" min="1" oninput="updatePurchaseQuote()">
      </div>
      <div style="background:var(--sur-var); border-radius:8px; padding:12px; font-size:13px; display:flex; justify-content:space-between;">
        <span>Total (@ $42/unit)</span>
        <strong id="purchase-total">$420</strong>
      </div>
    </div>`;
  document.getElementById('modal-footer').innerHTML = `
    <button class="btn-secondary" onclick="closeModal()">Cancel</button>
    <button class="btn-primary" onclick="submitPurchaseVouchers()">Pay & Add to Inventory</button>`;
  document.getElementById('modal-overlay').classList.remove('hidden');
}

function updatePurchaseQuote() {
  const qty = Math.max(1, parseInt(document.getElementById('purchase-qty').value) || 0);
  document.getElementById('purchase-total').textContent = '$' + (qty * 42);
}

async function submitPurchaseVouchers() {
  const qty = Math.max(1, parseInt(document.getElementById('purchase-qty').value) || 0);
  const type = document.getElementById('purchase-type').value;
  try {
    const res = await apiFetch('/vouchers/purchase', { method: 'POST', body: { qty, type } });
    if (res.error) { pushToast(res.message || res.error, 'error'); return; }
    pushToast(`Purchased ${res.qty} voucher(s) for $${res.total}`, 'success');
    refreshNotifications();
    closeModal();
  } catch (e) {
    pushToast('Failed to purchase vouchers', 'error');
  }
}

// Dashboard / sessions entry point for requesting or purchasing vouchers
function requestOrPurchaseVouchers() {
  if (canI('purchaseVouchers')) {
    showPurchaseVoucherModal();
  } else {
    pushToast('Voucher request sent to your organization admin', 'success');
  }
}
