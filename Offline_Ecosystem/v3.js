...
const v3App = {
  ...
  authStartRegistration() {
    document.getElementById('auth-overlay').classList.add('registration-mode');
  ...
  // Self-contained YouTube-style training player — no external dependency, so the
  // user is never stuck behind a video that fails to load. Clicking anywhere on
  // the player toggles play/pause; watching to the end unlocks the next step.
  _regVideo: { t: 0, playing: false, timer: null },
  _fmtVideoTime(s) {
  ...
  authToggleVideo(e) {
    ...
    const v = this._regVideo;
    this._setRegVideoPlaying(!v.playing);
  },
  _setRegVideoPlaying(play) {
    const player = document.getElementById('reg-video-player');
    const icon = document.getElementById('yt-playpause-icon');
    const progress = document.getElementById('yt-progress');
    const knob = document.getElementById('yt-knob');
    const curEl = document.getElementById('yt-current');
    if (!player) return;
    const DURATION = 150; // matches the "2:30" label
    const v = this._regVideo;
    ...
    if (play) {
      ...
      v.timer = setInterval(() => {
        // Accelerated so the demo doesn't take a real 2.5 minutes.
        v.t += 2;
        if (v.t >= DURATION) {
          v.t = DURATION;
        ...
        const pct = (v.t / DURATION) * 100;
        if (progress) progress.style.width = pct + '%';
        if (knob) knob.style.left = pct + '%';
        if (curEl) curEl.textContent = this._fmtVideoTime(v.t);
      }, 100);
    } else {
      v.playing = false;
      if (v.timer) { clearInterval(v.timer); v.timer = null; }
      // Restart from the beginning if the video had finished.
      if (v.t >= DURATION) {
        v.t = 0;
        if (progress) progress.style.width = '0%';
        if (knob) knob.style.left = '0%';
        if (curEl) curEl.textContent = '0:00';
        this._setRegVideoPlaying(true);
        return;
      }
      player.classList.add('paused');
  ...
  // Live counts on the Candidate Directory filter chips.
  updateCandidateCounts() {
    const cands = this.state.candidates || [];
    const set = (id, n) => { const el = document.getElementById(id); if (el) el.textContent = n; };
    set('cc-all', cands.length);
    set('cc-enrolled', cands.filter(c => c.examStatus !== 'in_progress' && c.examStatus !== 'completed').length);
    set('cc-in_progress', cands.filter(c => c.examStatus === 'in_progress').length);
    set('cc-completed', cands.filter(c => c.examStatus === 'completed').length);
  },
  ...
  renderCandidatesList(filtered) {
    ...
    if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state">
        ...
        <div class="es-sub">No candidates match this filter. Try another tab or add a candidate.</div>
      </div></td></tr>`;
      return;
    }
  ...
  filterSessions(status) {
    // Mirror the Candidate Directory chip behaviour so both screens feel identical.
  ...
  // Live counts on the Class Management filter chips.
  updateSessionCounts() {
  ...
  setSessionViewMode(mode) {
    ...
    // Update segmented-toggle button states
    ...
    // Re-render
    const currentFilter = document.querySelector('#session-filters .filter-chip.active');
    let status = 'all';
        ...
        return `
          <div class="card session-card">
            ...
            <div>
              ...
              <span class="sc-batch">2026 · Batch B1</span>
            </div>
            <div class="sc-meta">
