...
function changePanel(panelId) {
  ...
  if (target) {
    ...
    if (panelId === "dashboard-panel") {
      ...
      renderEarningsPanel();
      renderPaymentsPanel();
    } else if (panelId === "classmgmt-panel") {
      if (window.CCM) { CCM.filterSessions('all'); CCM.filterCandidates('all'); }
    }
  }
}

// --------------------------------------------------------------------------
// 5b. VOUCHER REDEMPTION (in-house proctor enters codes on exam day)
// --------------------------------------------------------------------------
async function submitVoucherRedeem() {
  const codeEl = document.getElementById("redeem-code-input");
  const sessEl = document.getElementById("redeem-session-input");
  const btn = document.getElementById("redeem-submit-btn");
  const result = document.getElementById("redeem-result");
  const code = (codeEl ? codeEl.value : "").trim().toUpperCase();
  if (code.length < 4) {
    if (result) result.innerHTML = redeemMsg("error", "Enter a valid voucher code.");
    return;
  }
  const sessionId = (sessEl ? sessEl.value : "").trim();
  const orig = btn ? btn.innerHTML : "";
  if (btn) { btn.disabled = true; btn.innerHTML = `<i class="material-icons spin">sync</i> Verifying…`; }
  try {
    const res = await fetch(`/api/vouchers/${encodeURIComponent(code)}/redeem`, {
      method: "POST",
    ...
    if (!res.ok || !data.success) {
      if (result) result.innerHTML = redeemMsg("error", data.message || "Could not redeem this voucher.");
    } else {
      const v = data.voucher;
      if (result) result.innerHTML = redeemMsg("success", `${v.voucherId} redeemed (${v.currentType}). Exam released.`);
      addRedeemLogEntry(v);
      if (codeEl) codeEl.value = "";
      if (sessEl) sessEl.value = "";
      if (typeof pushToast === "function") pushToast("Voucher redeemed", `${v.voucherId} verified and exam released.`, "success");
    }
  } catch (e) {
    if (result) result.innerHTML = redeemMsg("error", "Network error — is the API server running?");
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = orig; }
  }
}

function redeemMsg(kind, text) {
  const ok = kind === "success";
  const bg = ok ? "rgba(20,108,46,0.10)" : "rgba(179,38,30,0.10)";
  const bd = ok ? "rgba(20,108,46,0.30)" : "rgba(179,38,30,0.30)";
  const fg = ok ? "#146c2e" : "#b3261e";
  const ic = ok ? "check_circle" : "error";
  return `<div style="display:flex; align-items:center; gap:8px; padding:10px 12px; border-radius:8px; background:${bg}; border:1px solid ${bd}; color:${fg}; font-size:13px;"><i class="material-icons" style="font-size:18px;">${ic}</i><span>${text}</span></div>`;
}

function addRedeemLogEntry(v) {
  const log = document.getElementById("redeem-log");
  if (!log) return;
  if (log.dataset.empty !== "false") { log.innerHTML = ""; log.dataset.empty = "false"; }
  const row = document.createElement("div");
  row.style.cssText = "display:flex; justify-content:space-between; align-items:center; gap:8px; padding:10px 12px; border:1px solid var(--out-var); border-radius:8px; background:var(--md-bg);";
  row.innerHTML = `<span style="font-family:'JetBrains Mono', monospace; font-weight:700; font-size:13px; color:var(--on-sur);">${v.voucherId}</span>
    <span style="font-size:12px; color:var(--out); flex:1; text-align:center;">${v.currentType}${v.sessionId ? " · " + v.sessionId : ""}</span>
    <span class="bdg" style="background:#e6f4ea; color:#146c2e; font-size:11px;">Redeemed</span>`;
  log.prepend(row);
}
...
function renderDashboardRoster() {
  ...
  for (let i = 0; i < activeFeedsCount; i++) {
    ...
    const badgeText = c.actionStatus === "Suspended" ? 'deactivated' : (c.status === 'danger' ? 'critical' : (c.status === 'warning' ? 'warning' : 'optimal'));
...
function renderAgendaList() {
  ...
  sortedShifts.forEach(s => {
    ...
    let desc = "Open shift pool assignment";
...
function quickSnoozeCandidate(candidateId) {
  ...
  pushToast("Alert Snoozed", `Violations for ${c.name} snoozed for 60s.`, "warning");
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; background: var(--sur-var); border: 1px solid var(--out); border-radius: 8px; padding: 10px 14px; font-size: 11px;">
      <div style="display: flex; align-items: center; gap: 8px;">
        ...
        <div>
          ...
          <span style="opacity: 0.7;">(${alert.candidateId}) · Snoozed at ${alert.time}</span>
...
function toggleMetricHelp(metricKey) {
  ...
  switch (metricKey) {
    ...
    case 'integrity-score':
      ...
      metricDesc = `
        <p><strong>Definition:</strong> A predictive percentage indicating the cumulative honesty and rule compliance of the class.</p>
...
function applyCorporateTheme() {
  ...
  document.documentElement.style.setProperty('--pri-dk', '#111113');
...
// ==========================================================================
// CLASS & CANDIDATE MANAGEMENT (shared CCM module) — proctor host wiring
// ==========================================================================
// Separate from the live-monitoring CANDIDATES array so monitoring is untouched.
// Seed: 3 classes (in-class only / online-org / online-student) + a few
// candidates demonstrating class-default inheritance and per-candidate override.
let PROCTOR_CLASSES = [
  { id: 'pc_1', name: 'Food Safety Manager — Spring Cohort', program: 'Food Safety Manager',
    status: 'upcoming', createdAt: '2026-06-01', examDate: '2026-07-10', candidateCount: 0, readiness: 40,
    allowRetake: false, allowOnline: false, onlinePayer: null },
  { id: 'pc_2', name: 'HACCP Certification — Evening', program: 'HACCP Certification',
    status: 'ongoing', createdAt: '2026-05-20', examDate: '2026-06-30', candidateCount: 0, readiness: 70,
    allowRetake: true, allowOnline: true, onlinePayer: 'organization' },
  { id: 'pc_3', name: 'ServSafe Food Handler — Online Track', program: 'ServSafe Food Handler',
    status: 'ongoing', createdAt: '2026-05-15', examDate: '2026-06-28', candidateCount: 0, readiness: 60,
    allowRetake: true, allowOnline: true, onlinePayer: 'student' }
];
let PROCTOR_CANDIDATES = [
  { id: 'pcand_1', candidateId: 'SDC-CAN-A4Q7', name: 'Maria Gonzalez', email: 'maria.g@example.com',
    examAssessment: 'HACCP Certification', sessionId: 'pc_2', accommodation: { enabled: false, note: '' },
    rules: {}, voucherCode: 'VCH-C2041', voucherStatus: 'assigned', examStatus: 'enrolled', learningProgress: 30 },
  { id: 'pcand_2', candidateId: 'SDC-CAN-B8XK', name: 'Tom Becker', email: 'tom.becker@example.com',
    examAssessment: 'HACCP Certification', sessionId: 'pc_2', accommodation: { enabled: true, note: 'Extra time — dyslexia.' },
    rules: { examMode: 'online', onlinePayer: 'student' }, voucherCode: 'VCH-O2042', voucherStatus: 'assigned', examStatus: 'enrolled', learningProgress: 55 },
  { id: 'pcand_3', candidateId: 'SDC-CAN-C1MP', name: 'Aisha Rahman', email: 'aisha.r@example.com',
    examAssessment: 'ServSafe Food Handler', sessionId: 'pc_3', accommodation: { enabled: false, note: '' },
    rules: {}, voucherCode: 'VCH-O2050', voucherStatus: 'redeemed', examStatus: 'completed', learningProgress: 100 }
];
// Keep seeded class counts honest with the seeded roster.
PROCTOR_CLASSES.forEach(cl => { cl.candidateCount = PROCTOR_CANDIDATES.filter(c => c.sessionId === cl.id).length; });

function initProctorCCM() {
  if (!window.CCM) return;
  CCM.init({
    getSessions: () => PROCTOR_CLASSES,
    getCandidates: () => PROCTOR_CANDIDATES,
    showToast: (m, t) => pushToast(
      t === 'error' ? 'Error' : (t === 'info' ? 'Notice' : 'Success'),
      m, t === 'error' ? 'high' : (t === 'info' ? 'medium' : 'success')),
    showView: (v) => {
      if (v === 'session-detail') changePanel('session-detail-panel');
      else if (v === 'monitoring') changePanel('live-panel');
      else changePanel('classmgmt-panel'); // 'sessions' | 'candidates'
    },
    openAddCandidate: (sessionId) => openCCMModal('Add Candidate', CCM.addCandidateFormHtml(sessionId)),
    openAddClass: () => openCCMModal('Add Class', proctorClassFormHtml()),
    commitCandidate: (cand) => proctorCommitCandidate(cand),
    manageCandidate: (id) => {
      const c = PROCTOR_CANDIDATES.find(x => x.id === id);
      if (c) pushToast(c.name, `ID ${c.candidateId} · Voucher ${c.voucherCode || '—'} (${c.voucherStatus}).`, "medium");
    }
  });
}

function proctorCommitCandidate(cand) {
  const fromSession = CCM.currentSessionId;
  PROCTOR_CANDIDATES.unshift(cand);
  if (cand.sessionId) {
    const cls = PROCTOR_CLASSES.find(c => c.id === cand.sessionId);
    if (cls) cls.candidateCount = (cls.candidateCount || 0) + 1;
  }
  closeCCMModal();
  CCM.filterCandidates('all');
  CCM.filterSessions('all');
  if (fromSession && cand.sessionId === fromSession) CCM.openSessionDetail(fromSession);
}

// Compact Add-Class form (class rules become the inherited defaults).
function proctorClassFormHtml() {
  return `
    <div class="form-group"><label>Class Name</label><input type="text" class="form-control" id="pcc-name" placeholder="e.g. Food Safety — Fall Cohort"></div>
    <div class="form-group"><label>Certificate / Program</label><input type="text" class="form-control" id="pcc-program" placeholder="e.g. Food Safety Manager"></div>
    <div class="form-group"><label>Allow Retake Exam</label>
      <select class="form-control" id="pcc-retake"><option value="no">No</option><option value="yes">Yes</option></select></div>
    <div class="form-group"><label>Allow Online Exam</label>
      <select class="form-control" id="pcc-online" onchange="document.getElementById('pcc-payer-wrap').style.display=this.value==='yes'?'block':'none'">
        <option value="no">No</option><option value="yes">Yes</option></select></div>
    <div class="form-group" id="pcc-payer-wrap" style="display:none;"><label>Who bears the online price (default)?</label>
      <select class="form-control" id="pcc-payer"><option value="organization">Organisation</option><option value="student">Student</option></select></div>
    <div class="form-group"><label>Exam Date</label><input type="date" class="form-control" id="pcc-date"></div>
    <button class="btn btn-primary" style="width:100%; justify-content:center;" onclick="proctorSubmitClass()">Create Class</button>`;
}

function proctorSubmitClass() {
  const v = (id) => (document.getElementById(id) || {}).value || '';
  const allowOnline = v('pcc-online') === 'yes';
  const cls = {
    id: 'pc_' + Date.now(), name: v('pcc-name') || 'New Class', program: v('pcc-program'),
    status: 'draft', createdAt: new Date().toISOString().split('T')[0], examDate: v('pcc-date') || 'TBD',
    candidateCount: 0, readiness: 0,
    allowRetake: v('pcc-retake') === 'yes', allowOnline: allowOnline,
    onlinePayer: allowOnline ? (v('pcc-payer') || 'organization') : null
  };
  PROCTOR_CLASSES.unshift(cls);
  closeCCMModal();
  CCM.filterSessions('all');
  pushToast('Class created', `${cls.name} added as Draft.`, 'success');
}

// Lightweight centered modal for CCM forms (proctor side).
function openCCMModal(title, innerHtml) {
  closeCCMModal();
  const overlay = document.createElement('div');
  overlay.id = 'ccm-modal-overlay';
  overlay.className = 'ccm-scope';
  overlay.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.5); backdrop-filter:blur(4px); z-index:10000; display:flex; align-items:flex-start; justify-content:center; overflow:auto; padding:48px 16px;';
  overlay.addEventListener('click', (e) => { if (e.target === overlay) closeCCMModal(); });
  overlay.innerHTML = `
    <div class="card" style="width:480px; max-width:100%; background:var(--sur); color:var(--on-sur); border:1px solid var(--out); border-radius:16px; padding:24px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; border-bottom:1px solid var(--out-var); padding-bottom:12px;">
        <h3 style="margin:0; font-size:18px; font-weight:700;">${title}</h3>
        <button class="icon-button" onclick="closeCCMModal()"><i class="material-icons">close</i></button>
      </div>
      <div id="ccm-modal-body">${innerHtml}</div>
    </div>`;
  document.body.appendChild(overlay);
}
function closeCCMModal() {
  const el = document.getElementById('ccm-modal-overlay');
  if (el) el.remove();
}
