...
import { preExamChecks, proctorSessions, proctorIncidents, examAttempts } from "../../drizzle/schema";
...
export const proctoringRouter = router({
  ...
  dashboardStats: proctorRoleProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return { active: 0, flagged: 0, completed: 0, totalIncidents: 0 };

    const sessions = await db.select().from(proctorSessions)
      .orderBy(desc(proctorSessions.createdAt)).limit(200);

    const active = sessions.filter(s => s.status === "active").length;
    const flagged = sessions.filter(s => s.status === "flagged").length;
    const completed = sessions.filter(s => s.status === "completed").length;
    const totalIncidents = sessions.reduce((sum, s) => sum + (s.incidentCount || 0), 0);

    return { active, flagged, completed, totalIncidents };
  }),
});
