...
function initClock() {
  ...
  setInterval(() => {
    ...
    clockEl.innerHTML = `<i class="material-icons">schedule</i> ${month} ${day}, ${hrs}:${mins}:${secs} PST`;
  }, 1000);
}
...
function renderReportsCharts() {
  ...
  // Inject clean high-fidelity responsive SVG Donut Chart
  chartWrapper.innerHTML = `
    <svg width="100%" height="100%" viewBox="0 0 200 200">
      <!-- Donut Circle segments representing proportions -->
      <!-- Total Circumference = 2 * PI * r = 2 * 3.14 * 60 = 376.8 -->
      
      <!-- Background grey track -->
      <circle cx="100" cy="100" r="60" fill="none" stroke="${THEME === 'dark' ? '#2b2930' : '#f3edf7'}" stroke-width="24" />
      
      <!-- Gaze Deviation slice: 45% (dasharray: 169.5, gap: 207.3) -->
      <circle cx="100" cy="100" r="60" fill="none" stroke="var(--pri)" stroke-width="24" 
              stroke-dasharray="169.5 207.3" stroke-dashoffset="0" />
              
      <!-- Audio thresholds slice: 25% (dasharray: 94.2, gap: 282.6) -->
      <circle cx="100" cy="100" r="60" fill="none" stroke="var(--sec)" stroke-width="24" 
              stroke-dasharray="94.2 282.6" stroke-dashoffset="-169.5" />
              
      <!-- Double Person outline slice: 20% (dasharray: 75.3, gap: 301.5) -->
      <circle cx="100" cy="100" r="60" fill="none" stroke="var(--err)" stroke-width="24" 
              stroke-dasharray="75.3 301.5" stroke-dashoffset="-263.7" />
              
      <!-- Tab switches: 10% (dasharray: 37.8, gap: 339) -->
      <circle cx="100" cy="100" r="60" fill="none" stroke="var(--inf)" stroke-width="24" 
              stroke-dasharray="37.8 339" stroke-dashoffset="-339" />
              
      <text x="100" y="98" font-family="'Inter'" font-size="22" font-weight="800" fill="var(--on-sur)" text-anchor="middle">38</text>
      <text x="100" y="115" font-family="'Inter'" font-size="9" font-weight="700" fill="var(--on-sur-var)" text-anchor="middle" letter-spacing="0.05em">TOTAL COMPROMISES</text>
    </svg>
  `;
  
  // Render custom Legend values list
  const legend = document.getElementById("violations-chart-legend");
  legend.innerHTML = `
    <div class="legend-item"><span class="legend-dot" style="background:var(--pri)"></span> Gaze Deviations (45%)</div>
    <div class="legend-item"><span class="legend-dot" style="background:var(--sec)"></span> Audio Spikes (25%)</div>
    <div class="legend-item"><span class="legend-dot" style="background:var(--err)"></span> Extra Profiles (20%)</div>
    <div class="legend-item"><span class="legend-dot" style="background:var(--inf)"></span> Browser Tab Switch (10%)</div>
  `;
}
...
function selectCalendarDate(dateStr) {
  ...
  // Update sidebar forms details
  document.getElementById("no-shift-selected").classList.add("hidden");
  const form = document.getElementById("shift-details-form");
  ...
  // Set default values inside selector
  const existingShift = SHIFTS.find(s => s.date === dateStr);
  if (existingShift) {
    document.getElementById("shift-type").value = existingShift.type;
    document.getElementById("shift-exam").value = existingShift.exam || "cs101";
    document.getElementById("shift-start").value = existingShift.start;
    document.getElementById("shift-end").value = existingShift.end;
  } else {
    // Reset to defaults
    document.getElementById("shift-type").value = "available";
    document.getElementById("shift-exam").value = "cs101";
    document.getElementById("shift-start").value = "09:00";
    document.getElementById("shift-end").value = "17:00";
  }
  
  toggleShiftTypeSelect();
}

function toggleShiftTypeSelect() {
  const type = document.getElementById("shift-type").value;
  const examGroup = document.getElementById("shift-session-details");
  if (type === "session") {
...
function closeShiftForm() {
  document.getElementById("shift-details-form").classList.add("hidden");
  document.getElementById("no-shift-selected").classList.remove("hidden");
}

function saveShift() {
  const type = document.getElementById("shift-type").value;
  const exam = document.getElementById("shift-exam").value;
  const start = document.getElementById("shift-start").value;
  const end = document.getElementById("shift-end").value;
  
  // Remove existing shift record on this date if any
  SHIFTS = SHIFTS.filter(s => s.date !== CALENDAR_SELECTED_DATE);
  
  const newShift = {
    date: CALENDAR_SELECTED_DATE,
    type: type,
  ...
  pushToast("Shift Saved", "Calendar availability log updated successfully.", "success");
...
function renderEarningsPanel() {
  const tbody = document.getElementById("earnings-shift-tbody");
  if (!tbody) return;
  tbody.innerHTML = "";
  
  EARNINGS_SHIFTS.forEach(s => {
    const tr = document.createElement("tr");
    tr.style.cssText = "border-bottom: 1px solid rgba(0,0,0,0.03);";
    tr.innerHTML = `
      <td style="padding: 12px 16px; font-family: 'JetBrains Mono'; font-weight: 700;">${s.id}</td>
      <td style="padding: 12px 16px; font-weight: 700; color: var(--on-sur);">${s.session}</td>
      <td style="padding: 12px 16px; font-family: 'JetBrains Mono';">${s.duration}</td>
      <td style="padding: 12px 16px; font-family: 'JetBrains Mono'; color: var(--out);">${s.node}</td>
      <td style="padding: 12px 16px; font-family: 'JetBrains Mono';">${s.rate}</td>
      <td style="padding: 12px 16px; text-align: right; font-weight: 700; font-family: 'JetBrains Mono'; color: var(--suc);">${s.payout}</td>
    `;
    tbody.appendChild(tr);
  });
}
  ...
  PAYMENTS.forEach(p => {
    ...
    if (matchesSearch && matchesStatus) {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: 12px 16px; font-family: 'JetBrains Mono'; font-weight: 700; color: var(--suc);">${p.amount}</td>
        <td style="padding: 12px 16px;">
          <span class="bdg" style="background:${bg}; color:${color}; font-size:9px; padding:2px 6px; text-transform:uppercase;">${p.status}</span>
        </td>
        <td style="padding: 12px 16px; text-align: right;">
          <button class="mdbtn btn-tonal" onclick="pushToast('Processing Statement...', 'Compiling PDF details for invoice ${p.id}...', 'info')" style="font-size: 10px; min-height: 24px; padding: 2px 8px;">
            <i class="material-icons" style="font-size: 12px;">download</i> ${p.action}
          </button>
        </td>
      `;
      tbody.appendChild(tr);
