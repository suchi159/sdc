const app = {
        ...
        try {
            ...
            if (sessionsRes.ok) {
                ...
                this.state.classes = sessionsData.map(s => ({
                    ...
                    // Assigned proctor (surfaced on the class card + Overview tab).
                    proctorId: s.proctorId || null,
                    proctorName: s.proctorName || null,
    ...
    // --- Edit Candidate Profile → Exam Configuration overrides ---
    editCfgRetakeToggle(cb, id) {
        const c = this.state.candidates.find(x => x.id === id);
        if (c) { c.rules = c.rules || {}; c.rules.retakeAllowed = !!cb.checked; }
        const modeSel = document.getElementById('ec2-retake-mode-' + id);
        if (modeSel) modeSel.disabled = !cb.checked;
        this.editCfgPayerSync(id);
        this.showToast(cb.checked ? 'Retake enabled (override).' : 'Retake disabled (override).', cb.checked ? 'success' : 'info');
    },

    // Payer choice only applies when retake is on AND the retake mode is online.
    editCfgPayerSync(id) {
        const cb = document.getElementById('ec2-retake-' + id);
        const modeSel = document.getElementById('ec2-retake-mode-' + id);
        const wrap = document.getElementById('ec2-payer-wrap-' + id);
        if (!wrap) return;
        wrap.style.display = (cb && cb.checked && modeSel && modeSel.value === 'online') ? 'block' : 'none';
    },

    editCfgIdVerify(cb, id) {
        const c = this.state.candidates.find(x => x.id === id);
        if (c) c.idVerified = !!cb.checked;
        if (cb.checked) this.showToast('ID Verified', 'success');
    },

    // Persist the basic candidate fields, refresh the directory, then close.
    saveCandidateEdits(id, btn) {
        const c = this.state.candidates.find(x => x.id === id);
        if (c) {
            const val = (elId) => { const el = document.getElementById(elId); return el ? el.value.trim() : undefined; };
            const name = val('edit-cand-name'); if (name) c.name = name;
            const email = val('edit-cand-email'); if (email) c.email = email;
            const roll = val('edit-cand-roll'); if (roll !== undefined) c.rollNo = roll;
        }
        // Exam-config overrides are already persisted on change; just refresh.
        const active = document.querySelector('#candidate-filters .filter-chip.active');
        this.filterCandidates(active ? active.getAttribute('data-filter') : 'all');
        btn.classList.add('loading');
        setTimeout(() => {
            btn.classList.remove('loading');
    ...
    // Exam Configuration: org-wide defaults inherited by classes & candidates.
    toggleRetakeFields() {
        const on = document.getElementById('ec-allow-retake')?.checked;
        ['ec-retake-mode', 'ec-online-payer'].forEach(id => {
            const el = document.getElementById(id);
            if (el) { el.disabled = !on; el.closest('.form-group').style.opacity = on ? '1' : '0.5'; }
        });
    },

    saveExamConfig() {
        const cfg = {
            examMode: document.getElementById('ec-exam-mode')?.value,
            passScore: +document.getElementById('ec-pass-score')?.value,
            duration: +document.getElementById('ec-duration')?.value,
            questionCount: +document.getElementById('ec-question-count')?.value,
            allowRetake: !!document.getElementById('ec-allow-retake')?.checked,
            retakeMode: document.getElementById('ec-retake-mode')?.value,
            onlinePayer: document.getElementById('ec-online-payer')?.value,
            idVerify: !!document.getElementById('ec-id-verify')?.checked,
            webcam: !!document.getElementById('ec-webcam')?.checked,
            screenRecord: !!document.getElementById('ec-screen-record')?.checked,
            lockdown: !!document.getElementById('ec-lockdown')?.checked,
            accommodations: !!document.getElementById('ec-accommodations')?.checked
        };
        try { localStorage.setItem('org_exam_config', JSON.stringify(cfg)); } catch (e) { /* ignore */ }
        this.showToast('Exam configuration saved.', 'success');
    },

    loadExamConfig() {
        let cfg = null;
        try { cfg = JSON.parse(localStorage.getItem('org_exam_config')); } catch (e) { /* ignore */ }
        if (cfg) {
            const set = (id, v) => { const el = document.getElementById(id); if (el) el.value = v; };
            const chk = (id, v) => { const el = document.getElementById(id); if (el) el.checked = v; };
            set('ec-exam-mode', cfg.examMode); set('ec-pass-score', cfg.passScore);
            set('ec-duration', cfg.duration); set('ec-question-count', cfg.questionCount);
            set('ec-retake-mode', cfg.retakeMode); set('ec-online-payer', cfg.onlinePayer);
            chk('ec-allow-retake', cfg.allowRetake); chk('ec-id-verify', cfg.idVerify);
            chk('ec-webcam', cfg.webcam); chk('ec-screen-record', cfg.screenRecord);
            chk('ec-lockdown', cfg.lockdown); chk('ec-accommodations', cfg.accommodations);
        }
        this.toggleRetakeFields();
    },
    ...
    openDrawer(type, id = null) {
        ...
        if (type === 'edit-candidate') {
            ...
            // Effective (class-default + candidate-override) exam rules.
            const eff = (window.CCM && CCM.effectiveRules)
                ? CCM.effectiveRules(cand)
                : { examMode: 'in-class', retakeAllowed: false, retakeMode: 'in-class', onlinePayer: 'organization' };
            const accomYes = !!(cand.accommodation && cand.accommodation.enabled);
            const payerVisible = eff.retakeAllowed && eff.retakeMode === 'online';
            const examCfgSection = `
                <div style="background: var(--sur-main); border: 1px solid var(--out); padding: 16px; border-radius: 8px; margin-top: 16px;">
                    <h4 style="font-size: 13px; font-weight: 600; margin: 0 0 4px; display: flex; align-items: center; gap: 6px;">
                        <span class="material-icons" style="font-size: 16px; color: var(--pri);">tune</span> Exam Configuration
                    </h4>
                    <p style="font-size: 11px; color: var(--text-tertiary); margin: 0 0 16px;">Overrides the class &amp; organization defaults for this candidate.</p>
                    <div class="form-group">
                        <label for="ec2-mode-${cand.id}">Exam Mode</label>
                        <select class="form-control" id="ec2-mode-${cand.id}" onchange="CCM.examModeChange(this, '${cand.id}')">
                    ...
                    <div class="form-group" style="display: flex; align-items: center; justify-content: space-between; gap: 16px;">
                        ...
                        <label class="switch"><input type="checkbox" id="ec2-retake-${cand.id}" ${eff.retakeAllowed ? 'checked' : ''} onchange="app.editCfgRetakeToggle(this, '${cand.id}')"><span class="switch-slider"></span></label>
                    </div>
                    <div class="form-group">
                        <label for="ec2-retake-mode-${cand.id}">Retake Mode</label>
                        <select class="form-control" id="ec2-retake-mode-${cand.id}" ${eff.retakeAllowed ? '' : 'disabled'} onchange="CCM.retakeModeChange(this, '${cand.id}'); app.editCfgPayerSync('${cand.id}')">
                    ...
                    <div class="form-group" id="ec2-payer-wrap-${cand.id}" style="display: ${payerVisible ? 'block' : 'none'};">
                        <label for="ec2-payer-${cand.id}">Who Pays for Online Retake?</label>
                        <select class="form-control" id="ec2-payer-${cand.id}" onchange="CCM.payerChange('${cand.id}', this.value)">
                    ...
                    <div class="form-group">
                        <label for="ec2-accom-${cand.id}">Accommodation</label>
                        <select class="form-control" id="ec2-accom-${cand.id}" onchange="CCM.accommodationChange(this, '${cand.id}')">
                            <option value="NO" ${accomYes ? '' : 'selected'}>No</option>
                            <option value="YES" ${accomYes ? 'selected' : ''}>Yes</option>
                        </select>
                    </div>
                    <div class="form-group" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 0;">
                        <label style="margin: 0;">ID Verified</label>
                        <label class="switch"><input type="checkbox" id="ec2-idverify-${cand.id}" ${cand.idVerified ? 'checked' : ''} onchange="app.editCfgIdVerify(this, '${cand.id}')"><span class="switch-slider"></span></label>
                    </div>
                </div>`;
            ...
            content.innerHTML = `
                ...
                <div style="display: flex; gap: 12px; margin-top: 24px;">
                    ...
                    <button class="btn-primary" style="flex: 2; justify-content: center;" onclick="app.saveCandidateEdits('${cand.id}', this)">
                        Save Changes
                    </button>
                </div>
            `;
