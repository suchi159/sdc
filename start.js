...
const requestHandler = async (req, res) => {
  ...
  if (req.headers.host && req.headers.host.includes('3007')) {
    defaultHtml = 'v3.html';
  } else if (port === 3003) {
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    if (port === 3003) {
      res.setHeader('Content-Security-Policy', "default-src * data: blob: 'unsafe-inline'; script-src * 'unsafe-inline'; connect-src * 'unsafe-inline'; img-src * data: blob: 'unsafe-inline'; frame-src *; style-src * 'unsafe-inline';");
    } else {
      res.setHeader('Content-Security-Policy', "default-src * data: blob: 'unsafe-inline'; script-src * 'unsafe-inline'; connect-src * 'unsafe-inline'; img-src * data: blob: 'unsafe-inline'; frame-src *; style-src * 'unsafe-inline';");
    }
